...
s $\beta_j,\gamma_j,\delta_j$ reálnými, $x^2+\gamma_j x+\delta_j$ ireducibilními a $\sum_{j=1}^rk_j+2\sum_{j=1}^s\ell_j=n$
 ...
 {\bf 4.2. Věta.} {\em Buď $J$ obor integrity \fix{a} mějme funkci $\nu:J\to\Nbb$ a pravidlo dělení se zbytkem
  \fix{pro} $a,b\neq 0$ a $b$  nedělící $a$,
  $$
  a=sb+r \qtq{s} \nu(r)\fix{<}\nu(b).
  $$
...
Definujeme-li $d_Y(x,y)=d(x,y)$ pro $x,y\in Y$ získáme na $Y$ metriku; tak vytvořený $(Y,d_Y)$
 ...
 $$
 \forall\epsilon>0\ \exists n_0\ \text{\fix{takové že}}\ \forall n\geq n_0,\ d(x_n,x)<\epsilon.
 $$
...
Položme
 ...
 {\bf 5.1.} Budte $(\fix{X_i},d_i)$, $i=1,\dots,n$, metrické prostory. Na kartézském součinu
 $$
 \prod_{i=1}^nX_i
 $$
 definujeme metriku
...
$D\sue \Ebb_n$, a  $\ve{g}:D'\to \Ebb_k$, $D\fix{'}\sue \fix{\Ebb_m}$ můžeme skládat
...
{\bf 3.3. Totální diferenciál.} Řekneme, že funkce $f$ má {\em totální diferenciál} v bodě  $\fix{\ve{a}}$ existuje-li funkce $\mu$ spojitá v okolí
...
$$
Ať už to překvapí \fix{nebo} ne, naznačuje to, že parciální derivace vyšších řádů možná nezávisí na pořadí derivování.  A je to v podstatě pravda -- pokud všechny derivace o které jde jsou spojité
...
$$
\fix{Potom} má $F$ derivaci v  $b$, totiž
$$
...
$$
(f\circ \ve{g})(t_1,\dots,t_r)=f(\ve{g}(\fix{\ve{t}}))=f(g_1(\fix{\ve{t}}),\dots,g_n(\fix{\ve{t}}))
$$
...
\end{equation}
a hledáme řešení $y_k=f_k(\ve{x})$ ($=\fix{f_k}(x_1,\dots,x_n)$).

\bigskip

{\bf 1.2.} I v nejjednodušších případech nemusíme mít nutně řešení, o \fix{jednoznačném} ani nemluvě.
...
\begin{itemize}
\item budeme muset mít nějaké body $\ve{x}^0$, $\ve{y}^0$ pro které $F_k(\ve{x}^0,\ve{y}^0) \fix{= 0}$ kterými začneme,
